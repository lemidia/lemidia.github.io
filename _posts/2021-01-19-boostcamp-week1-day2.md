...
데이터를 저장하기 위한 메모리 공간의 프로그래밍상 이름입니다.
`a = 5` or `b = 3`

professor = “Sungchul Choi” 의 의미는” : Professor에 Sungchul Choi를 넣어라.
정확히는 Professor라는 변수에 “Sungchul Choi“ 라는 값을 넣으라는 의미입니다.

수학식 2x+7y=14 에서변수는 x와 y를 의미함
프로그래밍에서 변수는 수학과 약간 다른 개념입니다.

프로그래밍에서는 변수는 값을 저장하는 장소이고, 변수는 메모리 주소를 가지고 있고 변수에 들어가는 값은 메모리 주소에 할당됩니다.
...
다음은 파이썬에서 문자열을 다룰 때 많이 쓰이는 빌트인 스트링 함수입니다.
...
지금 글을 쓰는 시점에서 위에 3개는 완료를 하였고, baseball game 프로그램도 제출해 8/9를 pass 하였습니다.
당일 자정 전 까지 정리 노트를 제출해야해, 일단 노트를 마저 다 완성하고 나서 baseball game 프로그램 한 가지 에러를 고쳐 마무리 할 예정입니다.
